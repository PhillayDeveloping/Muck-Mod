...
import net.mcreator.muckmod.world.biome.MuckBiomeBiome;
...
public class MuckmodModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, MuckmodMod.MODID);
	public static final RegistryObject<Biome> MUCK_BIOME = REGISTRY.register("muck_biome", () -> MuckBiomeBiome.createBiome());
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(MuckBiomeBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, MUCK_BIOME.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, MUCK_BIOME.getId()),
											MuckmodModBlocks.MUCK_GRASS_BLOCK.get().defaultBlockState(),
											MuckmodModBlocks.MUCK_DIRT_BLOCK.get().defaultBlockState(),
											MuckmodModBlocks.MUCK_DIRT_BLOCK.get().defaultBlockState()));
